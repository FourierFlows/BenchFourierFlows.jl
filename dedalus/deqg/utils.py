...
def random_noise(domain, amplitude=1, seed=1234):
...
def peakedisotropicspectrum(domain, k0=6, energy0=0.5, seed=1234):
    ...
    gshape = domain.dist.grid_layout.global_shape(scales=1)
    slices = domain.dist.grid_layout.slices(scales=1)
    return energy0*rand.standard_normal(gshape)[slices]
...
def bind_state_variables(obj, variables):
    for v in variables:
        setattr(obj, v, obj.solver.state[v])
